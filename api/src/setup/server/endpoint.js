...
export default function (server) {
  ...
  // API endpoint
  server.all(params.common.endpoint.url, [authentication], async (request, response) => {
    ...
    let operation = modules[request.body.operation] || modules[request.params.operation]

    if(operation) {
      try {
        // Execute operation
        // operationName({ params, fields, auth })
        const { data, message = 'Success.', success = true } = await operation({
